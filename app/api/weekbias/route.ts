...
export async function GET() {
  try {
    const { data, error } = await supabase.from('weekly_bias').select('*').order('created_at',{ascending:false}).limit(20);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.from('weekly_bias').insert(body).select().single();
